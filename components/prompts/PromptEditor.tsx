...
export function PromptEditor({ check }: { check: any }) {
  ...
  return (
    <div>
      ...
      <div className="flex items-center gap-2 mb-2">
        <select className="border rounded px-2 py-1 text-sm" value={selected?.id || ''} onChange={e => setSelected(templates.find(t => t.id === e.target.value) || null)}>
        ...
        <button
          ...
          onClick={async () => {
            await fetch(`/api/checks/${check.id}`, {
              method: 'PUT',
              body: JSON.stringify({ prompt_template_id: selected?.id, actual_prompt_used: custom }),
      ...
      <textarea className="w-full h-40 border rounded p-2 text-sm font-mono" value={custom} onChange={e => setCustom(e.target.value)} />
