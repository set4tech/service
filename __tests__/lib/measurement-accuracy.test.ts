...
import { generateMockPDF, TEST_PDF_SPECS, type MockPDFSpec } from '../fixtures/generate-mock-pdfs';
...
describe('Measurement Accuracy with Mock PDFs', () => {
  ...
  describe('Page Size Method Accuracy', () => {
    it('should accurately measure 24x36 sheet at 1/8" scale', async () => {
      ...
      for (const refLine of spec.referenceLines) {
        ...
        console.log(
          `✓ ${spec.name} - ${refLine.label}: Expected ${refLine.lengthInches}", Got ${calculatedInches!.toFixed(2)}"`
        );
      }
    });
    ...
    it('should accurately measure Letter size at 1:1 scale', async () => {
      ...
      for (const refLine of spec.referenceLines) {
        ...
        // eslint-disable-next-line no-console
        console.log(
          `✓ ${spec.name} - ${refLine.label}: ${refLine.lengthInches}" = ${pixelsDistance.toFixed(1)} pixels (expected ${expectedPixels})`
        );
      }
    });
  });
  ...
  describe('Known Length Method Accuracy', () => {
    it('should accurately calibrate using known length', async () => {
      ...
      for (let i = 1; i < spec.referenceLines.length; i++) {
        ...
        const calculatedInches = pixelsToInchesKnownLength(pixelsDistance, pixelsPerInch!);
        ...
        // eslint-disable-next-line no-console
        console.log(
          `✓ Known Length Method - ${testLine.label}: Expected ${testLine.lengthInches}", Got ${calculatedInches!.toFixed(2)}"`
        );
      }
    });
    ...
    it('should handle diagonal calibration lines', async () => {
      ...
      // eslint-disable-next-line no-console
      console.log(
        `✓ Diagonal calibration: ${diagonalLine.lengthInches}" at ${pixelsPerInch!.toFixed(2)} pixels/inch`
      );
    });
  });
  ...
  describe('Edge Cases', () => {
    it('should handle small sheet sizes accurately', async () => {
      ...
      for (const refLine of spec.referenceLines) {
        ...
        // eslint-disable-next-line no-console
        console.log(
          `✓ Small sheet - ${refLine.label}: Expected ${refLine.lengthInches}", Got ${calculatedInches!.toFixed(2)}"`
        );
      }
    });
  ...
  describe('Consistency Tests', () => {
    it('should get same result with both calibration methods', async () => {
      ...
      const resultKnownLength = pixelsToInchesKnownLength(pixelsDistance, pixelsPerInch!);
      ...
      // eslint-disable-next-line no-console
      console.log(
        `✓ Both methods agree: Page Size = ${resultPageSize!.toFixed(2)}", Known Length = ${resultKnownLength!.toFixed(2)}", Expected = ${testLine.lengthInches}"`
      );
    });
  });
});
