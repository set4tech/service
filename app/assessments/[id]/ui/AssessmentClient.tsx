...
export default function AssessmentClient({ assessment, checks, progress }: Props) {
  ...
  const activeCheck = useMemo(() => checks.find(c => c.id === activeCheckId) || null, [checks, activeCheckId]);

  const [pdfUrl, setPdfUrl] = useState<string | null>(assessment?.pdf_url || null);
    ...
    <div className="grid grid-cols-2 h-full">
      <div className="border-r flex flex-col overflow-hidden">
        <div className="p-3 border-b">
          ...
          <div className="text-xs mt-1">{progress.completed} of {progress.totalChecks} checks</div>
        ...
        <div className="p-3 overflow-auto">
          {activeCheck ? (
            <>
              ...
              <div className="text-sm">Check: <span className="font-medium">{activeCheck.check_name}</span> {activeCheck.check_location ? `— ${activeCheck.check_location}` : null}</div>
