...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    for (const door of doors) {
      ...
      console.log(`[import-csv-doors] Creating door on page ${door.pageNumber}`);
      console.log(`[import-csv-doors] Measurements:`, door.measurements);

      // Always create new door - label will be auto-generated by database trigger
      const { data: elementInstance, error: createError } = await supabase
        .from('element_instances')
        .insert({
          assessment_id: assessmentId,
          element_group_id: elementGroup.id,
          label: null, // Trigger will auto-generate (e.g., "Doors 1", "Doors 2")
      ...
      if (sections && sections.length > 0) {
        ...
        const checksToInsert = sections
          ...
          .map(section => ({
            ...
            element_group_id: elementGroup.id,
            human_readable_title: `${elementInstance.label} - ${section.number}`,
            code_section_number: section.number,
            code_section_title: section.title,
          }));
      ...
      console.log(
        `[import-csv-doors] Created ${checksCreated} checks for ${elementInstance.label}`
      );

      createdDoors.push({
        id: elementInstance.id,
        instanceLabel: elementInstance.label, // Auto-generated label (e.g., "Doors 1")
