 ...
 *
 * Example:
 * /api/assessments/[id]?projectId=xxx&pageNumber=1&include=measurements,calibration,screenshots,pdf_scale
 */
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const promises: {
      ...
      calibration?: Promise<any | null>;
      screenshots?: Promise<any[]>;
      pdf_scale?: Promise<any | null>;
    } = {};
    ...
    if (shouldInclude('measurements') && projectId && pageNumber) {
      promises.measurements = (async () => {
        ...
        if (error) {
          console.error('[assessments/[id]] Error fetching measurements:', error);
          throw error;
        }
        return data || [];
      })();
    ...
    if (shouldInclude('calibration') && projectId && pageNumber) {
      promises.calibration = (async () => {
        ...
        if (error) {
          console.error('[assessments/[id]] Error fetching calibration:', error);
          throw error;
        }
        return data || null;
      })();
    ...
    if (shouldInclude('screenshots') && assessmentId) {
      promises.screenshots = (async () => {
        ...
        if (assignedError) {
          console.error('[assessments/[id]] Error fetching assigned screenshots:', assignedError);
          throw assignedError;
        }
        ...
        if (allScreenshotsError) {
          console.error('[assessments/[id]] Error fetching all screenshots:', allScreenshotsError);
    ...
    if (shouldInclude('pdf_scale') && assessmentId) {
      promises.pdf_scale = (async () => {
        ...
        if (error) {
          console.error('[assessments/[id]] Error fetching pdf_scale:', error);
          throw error;
        }
    ...
    const results = await Promise.all(
      Object.entries(promises).map(async ([key, promise]) => {
        try {
          ...
          console.error(`[assessments/[id]] Error fetching ${key}:`, error);
    ...
    console.error('[assessments/[id]] Unexpected error:', error);
    return NextResponse.json(
      {
        success: false,
