...
def clean_extracted_variables(variables):
    """Simplify to just essential values - remove confidence scores and nested structure."""
    cleaned = {}
        ...
        if isinstance(items, dict):
            ...
            for key, value in items.items():
                if value == 'none' or value == 'not_applicable':
                    # Skip not applicable items entirely
                    continue
                elif isinstance(value, dict) and 'value' in value:
                    # Extract just the value, no confidence score
                    cleaned_items[key] = value['value']
                    ...
                    for nested_key, nested_value in value.items():
                        if nested_value == 'none' or nested_value == 'not_applicable':
                            ...
                            nested_cleaned[nested_key] = nested_value['value']
                            if nested_value.get('confidence') == 'low':
                                print(f"  Warning: Low confidence for {category}.{key}.{nested_key}")
...
def main():
    ...
    variables["_metadata"] = {
        ...
        "extraction_method": "gemini-2.5-pro with simplified extraction (essential values only)",
