-- Fix bug where checks with NULL manual_status were being filtered out
-- This was causing element checks with AI analysis to not appear in reports
...
WHERE a.id = assessment_uuid
  ...
  -- FIX: Handle NULL manual_status correctly
