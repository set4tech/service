-- Fix: get_assessment_report was incorrectly filtering out checks with NULL manual_status
-- Bug was reintroduced in 20250117000001_add_calculation_table_to_checks.sql
-- The WHERE clause should include NULL manual_status and let AI analysis determine violations
...
WHERE a.id = assessment_uuid
  ...
  -- FIX: Include checks with NULL manual_status, let AI analysis determine violations
...
including checks with NULL manual_status that rely on AI analysis.';
