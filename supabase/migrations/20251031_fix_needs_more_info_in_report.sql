`-- Fix get_assessment_report to include needs_more_info in manual_status filter
-- This ensures manually overridden checks with needs_more_info status appear in violations report
...
WHERE a.id = assessment_uuid
  ...
  AND ((c.manual_status IN ('non_compliant', 'insufficient_information', 'needs_more_info')
